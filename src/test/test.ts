...
function execute(
  cmd: string
): Promise<{ output: string; error: number | undefined }> {
  return new Promise((resolve, reject) => {
    const child = child_process.exec(cmd, (err: any, result: any) => {
      resolve({ output: result, error: err ? err.code : undefined });
    });
    if (child.stdin) {
      child.stdin.end();
    } else {
...
describe("basho", () => {
  ...
  it(`Treats the input as yaml`, async () => {
    const echoProcess = child_process.spawn("echo", [
      "-e",
      "---\na:\n  b: hello\n",
    ]);
  ...
  it(`Prints error and does not exit with the printerror option`, async () => {
    ...
    output.should.equal(
      `[ \'a\', \'b\' ]\nError while evaluating expression: x.split(",").\nTypeError: x.split is not a function\n[ \'c\', \'d\' ]\n`
    );
  });
  ...
  it(`Handles a thrown error`, async () => {
    const { output, error } = await execute(
      `${basho} -j 'error("It did not work.")'`
    );
    should.exist(error);
    (error as any).should.equal(1);
    output.should.equal("It did not work.\n");
  });
